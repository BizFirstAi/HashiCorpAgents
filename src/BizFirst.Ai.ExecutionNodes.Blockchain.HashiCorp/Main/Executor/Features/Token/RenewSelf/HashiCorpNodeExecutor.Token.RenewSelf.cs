...
{
    private async Task<NodeExecutionResult> _HashiCorp_Token_RenewSelfAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenRenewSelfInfo
        if (mySettings?.ActiveInfo is not TokenRenewSelfInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call HashiCorp token service to extend this token's TTL
            var r = await _tokenService.RenewSelfAsync(ctx!, info.IncrementSeconds, cancellationToken);
            ...
            outputData["operation"] = mySettings.Operation ?? "renewSelf";
        ...
        catch (Exception ex)
        {
            ...
            return GetBuildErrorOutput_NodeExecutionResult_WithLogging("HASHICORP_UPSTREAM_ERROR", $"token/renewSelf failed: {ex.Message}", ex);
        }
    }
}
