...
{
    private async Task<NodeExecutionResult> _HashiCorp_Lease_LookupAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to LeaseLookupInfo
        if (mySettings?.ActiveInfo is not LeaseLookupInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call HashiCorp lease service to check remaining TTL / renewability
            var r = await _leaseService.LookupAsync(ctx!, info.LeaseID!, cancellationToken);
            ...
            //code-step: 1.5 - Extract lease info from result
            ...
            outputData["operation"] = mySettings.Operation ?? "lookup";
            outputData["errorCode"] = string.Empty;
            outputData["leaseID"]   = r.LeaseID;
            outputData["ttl"]       = r.Ttl;
            outputData["renewable"] = r.Renewable;

            //code-step: 1.7 - Convert lease info to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapSingleObjectIntoItems(lease, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, lease, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch unexpected exceptions and return error with context
            return GetBuildErrorOutput_NodeExecutionResult_WithLogging("HASHICORP_UPSTREAM_ERROR", $"lease/lookup failed for {info.LeaseID}: {ex.Message}", ex);
        }
    }
}
