...
{
    private async Task<NodeExecutionResult> _HashiCorp_Secrets_WriteAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to SecretsWriteInfo
        if (mySettings?.ActiveInfo is not SecretsWriteInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call HashiCorp secret service to write the secret
            var r = await _secretService.WriteAsync(ctx!, info.Path!, info.Data, info.Cas, cancellationToken);
            ...
            //code-step: 1.5 - Extract write confirmation record from result
            ...
            outputData["operation"] = mySettings.Operation ?? "write";

            //code-step: 1.7 - Convert write confirmation to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return GetBuildErrorOutput_NodeExecutionResult_WithLogging("HASHICORP_UPSTREAM_ERROR", $"secrets/write failed: {ex.Message}", ex);
        }
    }
}
