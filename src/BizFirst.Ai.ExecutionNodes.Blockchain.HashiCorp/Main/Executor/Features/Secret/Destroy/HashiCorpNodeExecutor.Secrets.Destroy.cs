...
{
    private async Task<NodeExecutionResult> _HashiCorp_Secrets_DestroyAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to SecretsDestroyInfo
        if (mySettings?.ActiveInfo is not SecretsDestroyInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call HashiCorp secret service to permanently destroy the secret version(s)
            var r = await _secretService.DestroyAsync(ctx!, info.Path!, info.Versions, cancellationToken);
            ...
            //code-step: 1.5 - Extract destroy confirmation record from result
            ...
            outputData["operation"] = mySettings.Operation ?? "destroy";

            //code-step: 1.7 - Convert destroy confirmation to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return GetBuildErrorOutput_NodeExecutionResult_WithLogging("HASHICORP_UPSTREAM_ERROR", $"secrets/destroy failed: {ex.Message}", ex);
        }
    }
}
