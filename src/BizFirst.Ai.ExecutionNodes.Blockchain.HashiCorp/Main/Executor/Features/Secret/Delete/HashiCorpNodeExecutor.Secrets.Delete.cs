...
{
    private async Task<NodeExecutionResult> _HashiCorp_Secrets_DeleteAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to SecretsDeleteInfo
        if (mySettings?.ActiveInfo is not SecretsDeleteInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call HashiCorp secret service to delete the secret version(s)
            var r = await _secretService.DeleteAsync(ctx!, info.Path!, info.Versions.Count == 0 ? null : info.Versions, cancellationToken);
            ...
            //code-step: 1.5 - Extract delete confirmation record from result
            ...
            outputData["operation"] = mySettings.Operation ?? "delete";

            //code-step: 1.7 - Convert delete confirmation to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return GetBuildErrorOutput_NodeExecutionResult_WithLogging("HASHICORP_UPSTREAM_ERROR", $"secrets/delete failed: {ex.Message}", ex);
        }
    }
}
