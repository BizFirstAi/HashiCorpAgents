...
{
    private async Task<NodeExecutionResult> _HashiCorp_Secrets_UndeleteAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to SecretsUndeleteInfo
        if (mySettings?.ActiveInfo is not SecretsUndeleteInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call HashiCorp secret service to undelete the secret version(s)
            var r = await _secretService.UndeleteAsync(ctx!, info.Path!, info.Versions, cancellationToken);
            ...
            //code-step: 1.5 - Extract undelete confirmation record from result
            ...
            outputData["operation"] = mySettings.Operation ?? "undelete";

            //code-step: 1.7 - Convert undelete confirmation to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return GetBuildErrorOutput_NodeExecutionResult_WithLogging("HASHICORP_UPSTREAM_ERROR", $"secrets/undelete failed: {ex.Message}", ex);
        }
    }
}
